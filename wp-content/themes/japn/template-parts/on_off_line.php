<?php /* Template Name: On_off_line */?>
<?php
get_header('no_transparent'); ?>
<main id="primary" class="site-main">
    <!-- CONTAIN_START -->
    <section id="contain">
        <?php get_sidebar('rightbar') ?>

        <div class="inner-page-main-ip">
            <div class="banner-block-ip">
                <div class="banner-block-overlay-ip">
                    <div class="container">
                        <div class="row">
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 banner-block-in-ip">
                                <div class="banner-middle-ip">
                                    <div class="breadcrumb-ip">
                                        <ol class="breadcrumb">
                                            <li class="breadcrumb-item"><a href="#">昼ナビTOP</a></li>
                                            <li class="breadcrumb-item"><a href="#">昼ナビとは</a></li>
                                            <li class="breadcrumb-item active">面談について</li>
                                        </ol>
                                    </div>
                                    <div class="inner-title-ip">
                                        <h2>面談について</h2>
                                    </div>
                                    <div class="clearfix"></div>
                                </div>
                                <div class="clearfix"></div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="clearfix"></div>
            </div>
            <div class="about-block-ip interview-block-op">
                <div class="container">
                    <div class="row">
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 about-block-in-ip">
                            <div class="about-middle-ip">
                                <div class="about-left-ip">
                                    <div class="interview-box-op">
                                        <div class="about-box-border-top-ip"></div>
                                        <div class="interview-box-middle-op">
                                            <div class="interview-title-op">
                                                <div class="interview-title-left-op">選べるオフライン・オンライン面談</div>
                                                <div class="interview-title-right-op">Follow-up Service</div>
                                            </div>
                                            <div class="interview-info-op">
                                                <p>オフライン／オンラインで面談を行う事が可能です。オフラインでの面談を推奨しており、キャリアプランの決定～面接対策までを1回の面談で行う事ができます。そのため、スピーディーに転職活動ができるので早く昼職デビューしたい方にピッタリです。昼職へ転職する上で不安な事や心配な点は全てお話してください！※オンラインでも面談を行っていますが、オフライン時とフローが異なりますのでご注意ください。
                                                </p>
                                            </div>
                                        </div>
                                        <div class="about-box-border-bottom-ip"></div>
                                    </div>
                                    <div class="interview-list-main-op">
                                        <h3>オフライン面談について</h3>
                                        <div class="about-extra-in-nip about-extra-in-full-nip">
                                            <div class="about-extra-title-nip">
                                                <h3>オフラインでの面談をおススメする理由</h3>
                                            </div>
                                            <div class="about-extra-desc-nip">
                                                ＊面談時に応募する企業の決定ができるのでスピーディーに転職活動が進む<br>
                                                ＊対面だからこそ分かる、1人1人の雰囲気を判断した上で仕事を紹介できる<br>
                                                ＊髪色や髪型、ネイルやメイクなど対面だから気付ける部分のフィードバックが可能
                                            </div>
                                        </div>
                                        <div class="interview-list-in-op">
                                            <div class="interview-list-op">
                                                <div class="interview-list-left-op">
                                                    <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/inter_img1.png"
                                                        alt="" />
                                                </div>
                                                <div class="interview-list-right-op">
                                                    <h4>1.キャリアプランの決定</h4>
                                                    <p>当社のキャリアアドバイザーと対面で面談を行います。電話ヒアリングの内容をもとに、「どんな仕事をしてみたいか」「どういう分野に興味があるのか」という事からお話をしていきます。また夜職での経験談など堅苦しくならず、ざっくばらんに何でもお話ししましょう。
                                                    </p>
                                                </div>
                                            </div>
                                            <div class="interview-list-op">

                                                <div class="interview-list-left-op">
                                                    <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/inter_img2.png"
                                                        alt="" />
                                                </div>
                                                <div class="interview-list-right-op">
                                                    <h4>2.応募する企業の決定</h4>
                                                    <p>ある程度将来のキャリアプランが固まったら、実際の求人票を一緒に確認しながら応募する企業を決めていきます。キャリアアドバイザーが感じた、あなたの雰囲気や強みを最大限発揮・活躍できそうなお仕事をご紹介していきます。
                                                    </p>
                                                </div>
                                            </div>
                                            <div class="interview-list-op">
                                                <div class="interview-list-left-op">
                                                    <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/inter_img3.png"
                                                        alt="" />
                                                </div>
                                                <div class="interview-list-right-op">
                                                    <h4>3.書類作成</h4>
                                                    <p>履歴書を書いた事がない方も安心！職務経歴書や履歴書は一緒に作成する事が可能です。あなたのアピールポイントがより魅力的に伝わるような文章の書き方を伝授します。あなたの魅力を言語化し、採用担当者の心に響く履歴書を創り上げていきましょう。
                                                    </p>
                                                </div>
                                            </div>
                                            <div class="interview-list-op">

                                                <div class="interview-list-left-op">
                                                    <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/inter_img4.png"
                                                        alt="" />
                                                </div>
                                                <div class="interview-list-right-op">
                                                    <h4>4.面接対策</h4>
                                                    <p>面談時に応募したい企業が決まったら面接の対策も行います。入退室時などの基本的なマナーから、転職理由や志望動機などの受け答えなどを一緒に考えていきましょう。また、昼職ならではのオフィスカジュアルやネイル、髪型についてのアドバイスも丁寧に行います。
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="interview-list-main-op">
                                        <h3>オンライン面談について</h3>
                                        <div class="interview-list-in-op">
                                            <div class="interview-list-op">
                                                <a href="#">
                                                    <div class="interview-list-left-op">
                                                        <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/inter_img5.png"
                                                            alt="" />
                                                    </div>
                                                    <div class="interview-list-right-op">
                                                        <p class="pt-0">
                                                            キャリアアドバイザーとオンライン上で面談を行う事も可能です。電話ヒアリングでお聞きした内容をさらに深めていき、今後の描きたいキャリアなどについてお話ししましょう。面談時にパソコン上で共有された求人票の内容だけなく、面談終了後にお送りするメールにて求人票をしっかり目を通す事が必要です。
                                                        </p>
                                                    </div>
                                                </a>
                                            </div>
                                        </div>

                                    </div>
                                    <div class="interview-btn-op">
                                        <a href="<?php echo get_site_url('')?>/registration_form" class="common-btn-hp">登録する</a>
                                    </div>
                                </div>
                                <?php get_sidebar('wishes') ?>
                                <div class="clearfix"></div>
                            </div>
                            <div class="clearfix"></div>
                        </div>
                    </div>
                </div>
                <div class="clearfix"></div>
            </div>
        </div>


        <?php get_footer('search'); ?>
        <div class="company-block-hp">
            <div class="container">
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 company-block-in-hp">
                        <div class="company-middle-hp">
                            <div class="company-middle-in-hp">
                                <div class="company-img-hp">
                                    <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/logo.png" alt="" />
                                </div>
                                <div class="company-info-hp">
                                    昼ナビは、キャバクラ・クラブ・ボーイ・黒服などのナイトワークの方々が昼職に転職に転職したいと考えた時に感じる「履歴書の書き方が分からない」・「自分にあった仕事が分からない」・「昼職の実情が分からない」といったお悩みをアドバイザーが一人ひとりに真剣に向き合い、お話しした上で全て解消し、希望や適正に応じたお仕事をご紹介します。
                                </div>
                            </div>
                        </div>
                        <div class="clearfix"></div>
                    </div>
                </div>
            </div>
            <div class="clearfix"></div>
        </div>
        <div class="clearfix"></div>
    </section>
    <!-- CONTAIN_END -->
</main>
<?php get_footer();